...
wrong_strike_per_token = defaultdict(lambda: 0)
...
def click(token: str):
    ...
    if "captcha_response" in body and isinstance(body["captcha_response"], (int, float)):
        ...
        if success:
            wrong_strike_per_token[token] = 0
            counter_by_token[token] -= 1
            ...
            wrong_strike_per_token[token] += 1
            if wrong_strike_per_token[token] % 5 == 0:
                counter_by_token[token] = min(counter_by_token[token] + 50, 2024)
            retry_captcha = True
